...
function interface.ColdFusion.new(
    ...
    local outlineColorState = _Computed(function(appearance : AppearanceData, state : Enums.ButtonState)
        local dynamicScheme = MaterialColor.getDynamicScheme(
            appearance.PrimaryColor, 
            appearance.SecondaryColor, 
            appearance.TertiaryColor, 
        ...
        return if state == Enums.ButtonState.Enabled then outlineVariant
            elseif state == Enums.ButtonState.Disabled then outline 
            elseif state == Enums.ButtonState.Hovered then outlineVariant 
            elseif state == Enums.ButtonState.Focused then onSurface 
            elseif state == Enums.ButtonState.Pressed then outlineVariant
            elseif state == Enums.ButtonState.Dragged then outlineVariant 
        else outlineVariant
   ...
   --[[ local trailingIcon = StandardIconButton.ColdFusion.new(
    ...
    _new("UIStroke")({
        Name = "Shadow",
        Parent = out:FindFirstChild("Main"),
        Thickness = 1,
